...
def split_train_test(dataset_path: os.path, save_path, rand = True, train_ratio = 0.8, mean_std_path=None):
    ...
    for file_name in file_list:
        ...
        class_list[int(action)-1][int(location)-1].append([file_n, action])

    train_list = []
    test_list = []

    for i, n_class in enumerate(class_list):
        for j, n_location in enumerate(n_class):
            train_num = int(train_ratio * len(n_location))
            print(f'class: {i} location: {j} class_len: {len(n_class)} location_len: {len(n_location)} train_len: {train_num}  test_len: {len(n_location)-train_num}')
            if rand:
                random.shuffle(n_location)
            train_list.extend(n_location[:train_num])
            test_list.extend(n_location[train_num:])
...
if __name__ == '__main__':
    ...
    check_dataset(train_list_path)
    check_dataset(test_list_path)
